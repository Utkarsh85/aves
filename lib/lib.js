...
var lib=
{
	process: function (config,argv) {
		...
		{
			if(config[templateName].hasOwnProperty('transformer') && typeof(config[templateName].transformer) == "function")
			{
				argv= config[templateName].transformer(argv);

				if(!argv)
				{
					throw 'Did you miss returning argv from the transformer function for template '+templateName+'\nPlease add return argv;';
				}
			}
